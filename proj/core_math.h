...
class core_math : public node, public drawable, public provider
{
private:
	bool have_new_mesh;
public:
...
public:
	core_math() : node("core_math")
	{

		generate_dini_surface();
	...
	void generate_xxx_surface() {

	}
	void on_set(void* member_ptr)
	{
		if (member_ptr == &show_vertices) {
			post_redraw();
			update_member(&show_vertices);
		}
	}
	void create_gui()
	{
		add_decorator("core_math", "heading", "level=2");
		add_member_control(this, "show vertex", show_vertices, "toggle");
		add_member_control(this, "show wireframe", show_wireframe, "toggle");
		add_member_control(this, "show surface", show_surface, "toggle");
...
cgv::base::object_registration<core_math> math("core_math");
